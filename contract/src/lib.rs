...
///
/// The contract maintains a list of board members (signers) and proposers.
/// Board members can both propose and sign actions.
/// Proposers can only propose actions.
/// Actions require a configurable quorum of valid signatures before execution.
    ...
    + multisig_events::MultisigEventsModule
{
    ...
    /// Board members can withdraw their signatures if they no longer desire
    /// for the action to be executed.
    /// Actions that are left with no valid signatures can be then deleted
    /// to free up storage.
    #[endpoint]
