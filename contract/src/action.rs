use multiversx_sc::{
    api::ManagedTypeApi,
    types::{
        BigUint, CodeMetadata, ManagedAddress, ManagedBuffer, ManagedVec,
        TokenIdentifier,
    },
};
...
/// Data for a direct ESDT token transfer.
#[type_abi]
#[derive(NestedEncode, NestedDecode, Clone)]
...
pub enum Action<M: ManagedTypeApi> {
    ...
    /// Direct ESDT/SFT/NFT transfer (single token).
    SendESDTTransfer(EsdtTransferData<M>),
    /// Staking delegation operation: delegate, undelegate, claim rewards, redelegate.
    StakingOp {
        provider: ManagedAddress<M>,
