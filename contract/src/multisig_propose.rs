use crate::action::{Action, CallActionData, EsdtTransferData, StakingOpType};

use multiversx_sc::imports::*;

/// Module for proposing actions.
...
pub trait MultisigProposeModule: crate::multisig_state::MultisigStateModule {
    ...
    /// Can call smart contract endpoints directly.
    /// Can use ESDTTransfer/ESDTNFTTransfer/MultiESDTTransfer to send tokens,
    /// while also optionally calling endpoints.
    ...
    /// Propose a direct ESDT/SFT/NFT token transfer.
    /// For fungible tokens, set token_nonce to 0.
    #[endpoint(proposeESDTTransfer)]
    fn propose_esdt_transfer(
        &self,
        to: ManagedAddress,
        ...
        self.propose_action(Action::SendESDTTransfer(EsdtTransferData {
            to,
            token_id,
            token_nonce,
            amount,
    ...
    /// Propose delegating EGLD to a staking provider.
    ...
    fn propose_delegate(
        ...
        self.propose_action(Action::StakingOp {
            provider,
            op_type: StakingOpType::Delegate,
            amount,
        })
    }

    /// Propose undelegating EGLD from a staking provider.
    ...
    fn propose_undelegate(
        ...
        require!(amount > 0, "undelegation amount must be greater than 0");
        self.propose_action(Action::StakingOp {
            provider,
            op_type: StakingOpType::Undelegate,
            amount,
        })
    }

    /// Propose claiming staking rewards from a provider.
    #[endpoint(proposeClaimRewards)]
    fn propose_claim_rewards(
        &self,
        provider: ManagedAddress,
    ) -> usize {
        self.propose_action(Action::StakingOp {
            provider,
            op_type: StakingOpType::ClaimRewards,
            amount: BigUint::zero(),
        })
    }

    /// Propose redelegating rewards to a staking provider.
    #[endpoint(proposeRedelegate)]
    fn propose_redelegate(
        &self,
        provider: ManagedAddress,
    ) -> usize {
        self.propose_action(Action::StakingOp {
            provider,
            op_type: StakingOpType::Redelegate,
            amount: BigUint::zero(),
        })
    }
}
