use crate::{
    action::{Action, ActionFullInfo, StakingOpType},
    user_role::UserRole,
};
...
/// Module for performing (executing) actions that have enough signatures.
    ...
    crate::multisig_state::MultisigStateModule + crate::multisig_events::MultisigEventsModule
{
    ...
    fn perform_action(&self, action_id: usize) -> OptionalValue<ManagedAddress> {
        ...
        match action {
            ...
            Action::SendESDTTransfer(esdt_data) => {
                // Direct ESDT/SFT/NFT transfer
                self.perform_esdt_transfer_event(
                    action_id,
                    &esdt_data.to,
                    &esdt_data.token_id,
                    esdt_data.token_nonce,
                    &esdt_data.amount,
                );
                self.tx()
                    .to(&esdt_data.to)
            ...
            Action::StakingOp {
                ...
                let gas = self.gas_for_transfer_exec();
                self.perform_staking_op_event(action_id, &provider, &amount);

                match op_type {
                    StakingOpType::Delegate => {
                        // Send EGLD to staking provider with "delegate" endpoint
                        self.tx()
                            .to(&provider)
                            .egld(&amount)
                            .gas(gas)
                            .raw_call(ManagedBuffer::from(b"delegate"))
                            .transfer_execute();
                    }
                    StakingOpType::Undelegate => {
                        // Call "unDelegate" on staking provider with amount as argument
                        self.tx()
                            .to(&provider)
                            .gas(gas)
                            .raw_call(ManagedBuffer::from(b"unDelegate"))
                            .argument(&amount)
                            .transfer_execute();
                    }
                    StakingOpType::ClaimRewards => {
                        self.tx()
                            .to(&provider)
                            .gas(gas)
                            .raw_call(ManagedBuffer::from(b"claimRewards"))
                            .transfer_execute();
                    }
                    StakingOpType::Redelegate => {
                        self.tx()
                            .to(&provider)
                            .gas(gas)
                            .raw_call(ManagedBuffer::from(b"reDelegateRewards"))
                            .transfer_execute();
                    }
                }
                OptionalValue::None
